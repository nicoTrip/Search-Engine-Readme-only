...
- **Frontend**
  Worked on weighting all scores to return the best pages. Our weights included pagerank, TF-IDF score, and a phrasal search
...
- Writing many small files was inefficient (due to 4kB disk page sizes), so we needed fewer, larger files
    - this also required us to have better caching so we didn't need to read the entire file repeatedly
- Appending to files reduced write time, but required careful file management
- Sending one message per URL or task slowed the networking so we aggressively batched all messages
